...
def test_defaults_target_luna_responses_workload() -> None:
    ...
    assert settings.history_window_seconds == 21600
    assert settings.followup_window_seconds == 600
...
def test_overrides_are_parsed_and_normalized() -> None:
    settings = Settings.from_env(
        {
            **VALID_ENV,
            ...
            "TRUBOT_HISTORY_WINDOW_MINUTES": "90",
            "TRUBOT_FOLLOWUP_WINDOW_MINUTES": "5",
    ...
    assert settings.history_window_seconds == 5400
    assert settings.followup_window_seconds == 300
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("TRUBOT_HISTORY_WINDOW_MINUTES", "0", "must be greater than 0"),
        ("TRUBOT_FOLLOWUP_WINDOW_MINUTES", "-1", "must be at least 0"),
